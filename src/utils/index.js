...
export const hoverIntent = function() {
	...
	for (let i = 0; i < items.length; i++) {
		...
		function mouseOverHandler() {
			...
			item.querySelector('.product-footer').setAttribute(
				'style',
...
export function getMinMaxPrice(variantsArray) {
	let min = Array.isArray(variantsArray)  ? Number(variantsArray[0].price) : 0;
	let max = 0;
	let minOld = Array.isArray(variantsArray)  ? Number(variantsArray[0].price) : 0;
	let maxOld = 0;
	if (variantsArray) {
